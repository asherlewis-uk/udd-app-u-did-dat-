...
- **Deployment reality (2026-04-18):** `api`, `gateway`, `orchestrator`, `session-reaper`, `usage-meter`, `ai-orchestration`, and `worker-manager` are all **ACTIVE / DEPLOYED** to Cloud Run (us-central1). `collaboration` is pending Pusher refactor and has no Cloud Build config. See [current-blockers-and-resolution-options.md](./current-blockers-and-resolution-options.md).
