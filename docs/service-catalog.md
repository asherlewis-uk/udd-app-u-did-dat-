...
| --------------------- | ------------------------------------------------------------------------------------- | ---------------------------------------------------- | -------- | -------------------------- |
| `apps/web`            | Primary hosted web client for auth, projects, sessions, previews, and AI surfaces. Canonical public host: `app.asherlewis.org` (reserved — no deploy target wired yet; see [implementation-gaps.md](./implementation-gaps.md#active-gaps--public-hosting-hardened-v1-canonical-hosts)). | `apps/api`, hosted auth flow, hosted preview gateway | Core     | Yes                        |
...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- | ---------------------------- | -------------------------- | ----------------- |
| `apps/api`              | Public API and BFF for web and iOS. Canonical public host: `api.asherlewis.org` (global HTTPS LB default backend).                                     | `packages/auth`, `packages/database`, `apps/orchestrator`, `apps/ai-orchestration` | Core                         | Yes                        | **ACTIVE / DEPLOYED** to Cloud Run (us-central1). Cloud Build config exists. Image published. |
| `apps/gateway`          | Hosted preview ingress and proxy. Reached via `api.asherlewis.org/preview/*` (LB path-routes `/preview/*` to this service).                             | `packages/auth`, `packages/database`                                               | Core                         | Yes                        | **ACTIVE / DEPLOYED** to Cloud Run (us-central1). Cloud Build config exists. Image published. |
