...
| `apps/worker-manager`   | Capacity snapshot ingestion and worker-capacity persistence (canonical model: always-on HTTP service; Terraform must be corrected from job to service) | `packages/database`                                                                | Core                         | Yes                        |
| `apps/host-agent`       | Runtime-host registration and heartbeat publication                                                                                                    | `apps/worker-manager`, runtime hosts                                               | Core                         | Yes                        |
| `apps/session-reaper`   | Idle-session cleanup and orphaned-lease cleanup (canonical model: scheduled single-invocation job; code must be refactored from interval loop)         | `packages/database`, `packages/events`                                             | Core                         | Yes                        |
