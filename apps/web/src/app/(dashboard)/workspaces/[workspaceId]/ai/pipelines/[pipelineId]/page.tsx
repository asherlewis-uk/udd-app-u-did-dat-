...
export default function PipelineDetailPage() {
  const params = useParams<{ workspaceId: string; pipelineId: string }>();
  const { workspaceId, pipelineId } = params;
  const router = useRouter();

  const { user } = useAuth();
  const token = user?.token ?? null;

  const { pipelines, isLoading: pipelinesLoading } = usePipelines(token, workspaceId);
  const { runs, isLoading: runsLoading, mutate: mutateRuns } = usePipelineRuns(token, workspaceId);
  ...
  const breadcrumbs = [
    { label: 'AI Hub', href: `/workspaces/${workspaceId}/ai` },
    ...
    return (
      <div className="flex flex-col">
        <PageHeader
          ...
          breadcrumbs={[{ label: 'AI Hub', href: `/workspaces/${workspaceId}/ai` }]}
          actions={
            <Button
              variant="secondary"
              size="sm"
              onClick={() => router.push(`/workspaces/${workspaceId}/ai`)}
            >
