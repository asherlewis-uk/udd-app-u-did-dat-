...
export default function WorkspacesPage() {
  ...
  const { workspaces, isLoading } = useWorkspaces(user?.token ?? null);
            ...
            {workspaces.map((ws) => (
              ...
              >
                ...
                <div className="flex-1 overflow-hidden">
                  <p className="font-semibold text-[#fafafa]">{ws.name}</p>
                  <p className="text-sm text-[#71717a]">
                    {ws.slug} · Updated {formatRelativeTime(ws.updatedAt)}
                  </p>
                </div>
