...
router.get('/workspaces/:id/ai/pipelines', requirePermission('ai.pipeline.read'), async (req, res, next) => {
  try {
...
router.post('/workspaces/:id/ai/pipelines', requirePermission('ai.pipeline.create'), async (req, res, next) => {
  try {
    const ctx = getContext();
    if (!await assertMember(ctx, req.auth!.userId, req.params['id']!)) {
      return res.status(404).json({ code: 'NOT_FOUND', correlationId: req.correlationId });
    ...
    if (!body.name || !body.pipelineDefinitionJson) {
      return res.status(400).json({ code: 'VALIDATION_ERROR', message: 'name and pipelineDefinitionJson are required', correlationId: req.correlationId });
    }

    // DAG validation — must pass before storage
    const dagResult = await validateDag(body.pipelineDefinitionJson, req.params['id']!, ctx.agentRoles);
    if (!dagResult.valid) {
      return res.status(400).json({ code: 'VALIDATION_ERROR', message: dagResult.errors.join('; '), errors: dagResult.errors, correlationId: req.correlationId });
    }
...
router.get('/workspaces/:id/ai/pipelines/:pipelineId', requirePermission('ai.pipeline.read'), async (req, res, next) => {
  try {
    const ctx = getContext();
    if (!await assertMember(ctx, req.auth!.userId, req.params['id']!)) {
      return res.status(404).json({ code: 'NOT_FOUND', correlationId: req.correlationId });
...
router.patch('/workspaces/:id/ai/pipelines/:pipelineId', requirePermission('ai.pipeline.update'), async (req, res, next) => {
  try {
    const ctx = getContext();
    if (!await assertMember(ctx, req.auth!.userId, req.params['id']!)) {
      return res.status(404).json({ code: 'NOT_FOUND', correlationId: req.correlationId });
    ...
    const { name, description, pipelineDefinitionJson, isActive } = req.body as Partial<Pick<PipelineDefinition, 'name' | 'description' | 'pipelineDefinitionJson' | 'isActive'>>;

    // Re-validate DAG if definition is changing
    if (pipelineDefinitionJson) {
      const dagResult = await validateDag(pipelineDefinitionJson, req.params['id']!, ctx.agentRoles);
      if (!dagResult.valid) {
        return res.status(400).json({ code: 'VALIDATION_ERROR', message: dagResult.errors.join('; '), errors: dagResult.errors, correlationId: req.correlationId });
      }
    }

    const updated = await ctx.pipelines.update(pipeline.id, { name, description, pipelineDefinitionJson, isActive });
...
router.delete('/workspaces/:id/ai/pipelines/:pipelineId', requirePermission('ai.pipeline.delete'), async (req, res, next) => {
  try {
    const ctx = getContext();
    if (!await assertMember(ctx, req.auth!.userId, req.params['id']!)) {
      return res.status(404).json({ code: 'NOT_FOUND', correlationId: req.correlationId });
