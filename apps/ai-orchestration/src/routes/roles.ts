...
router.get('/workspaces/:id/ai/roles', requirePermission('ai.role.read'), async (req, res, next) => {
  try {
...
router.post('/workspaces/:id/ai/roles', requirePermission('ai.role.create'), async (req, res, next) => {
  try {
    const ctx = getContext();
    if (!await assertMember(ctx, req.auth!.userId, req.params['id']!)) {
      return res.status(404).json({ code: 'NOT_FOUND', correlationId: req.correlationId });
    ...
    if (!body.name || !body.providerConfigId || !body.modelIdentifier) {
      return res.status(400).json({ code: 'VALIDATION_ERROR', message: 'name, providerConfigId, modelIdentifier are required', correlationId: req.correlationId });
    }
...
router.get('/workspaces/:id/ai/roles/:agentRoleId', requirePermission('ai.role.read'), async (req, res, next) => {
  try {
    const ctx = getContext();
    if (!await assertMember(ctx, req.auth!.userId, req.params['id']!)) {
      return res.status(404).json({ code: 'NOT_FOUND', correlationId: req.correlationId });
...
router.patch('/workspaces/:id/ai/roles/:agentRoleId', requirePermission('ai.role.update'), async (req, res, next) => {
  try {
    const ctx = getContext();
    if (!await assertMember(ctx, req.auth!.userId, req.params['id']!)) {
      return res.status(404).json({ code: 'NOT_FOUND', correlationId: req.correlationId });
    ...
    const { name, description, modelIdentifier, endpointOverrideUrl, roleConfigJson, isActive } = req.body as Partial<Pick<AgentRole, 'name' | 'description' | 'modelIdentifier' | 'endpointOverrideUrl' | 'roleConfigJson' | 'isActive'>>;
    const updated = await ctx.agentRoles.update(role.id, { name, description, modelIdentifier, endpointOverrideUrl, roleConfigJson, isActive });
...
router.delete('/workspaces/:id/ai/roles/:agentRoleId', requirePermission('ai.role.delete'), async (req, res, next) => {
  try {
    const ctx = getContext();
    if (!await assertMember(ctx, req.auth!.userId, req.params['id']!)) {
      return res.status(404).json({ code: 'NOT_FOUND', correlationId: req.correlationId });
