...
router.get('/projects/:id/comments', requirePermission('comment.read'), async (req, res, next) => {
  try {
    ...
    if (!project) return res.status(404).json({ code: 'NOT_FOUND', correlationId: req.correlationId });

    const membership = await memberships.findByUserAndWorkspace(req.auth!.userId, project.workspaceId);
...
router.post('/projects/:id/comments', requirePermission('comment.create'), async (req, res, next) => {
  try {
    ...
    if (!body) return res.status(400).json({ code: 'VALIDATION_ERROR', message: 'body is required', correlationId: req.correlationId });
